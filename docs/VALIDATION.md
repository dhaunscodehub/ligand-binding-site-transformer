...
| 10 | **Dead `isfinite` branch in the JSON encoder** (inherited pattern). `np.float64` is a `float` subclass, so the C encoder handles it and `allow_nan=False` raises before `default()` is consulted. | A serialisation smoke check | Non-finite handling moved to a pre-dump `sanitise()` walk. |
